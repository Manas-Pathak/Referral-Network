...
1. **BFS for Reach Calculation**: Chose BFS over DFS for calculating total referrals as it naturally processes nodes level by level, making it easier to understand and debug. Time complexity: O(V + E).

2. **Greedy Algorithm for Unique Reach**: Implemented the greedy approach as specified, which provides a good approximation for the NP-hard set cover problem. Time complexity: O(V²).

3. **All-Pairs Shortest Path for Flow Centrality**: Used BFS from each node to compute shortest distances, then applied the mathematical property that a node v lies on a shortest path between s and t if `dist(s, v) + dist(v, t) == dist(s, t)`. Time complexity: O(V³).

4. **Binary Search for Bonus Optimization**: Used binary search over the bonus space to efficiently find the minimum required bonus, leveraging the monotonic nature of the adoption probability function. Time complexity: O(log B) where B is the bonus range.

### Implementation Details

**Top Referrers Selection (Part 2):**
The `get_top_referrers(k)` method returns the top k referrers based on total referral count. **How to pick appropriate k**: 
...
**Network Growth Simulation Parameters (Part 4):**
The simulation uses fixed parameters as specified in the assignment:
...
**Flow Centrality Implementation (Part 3):**
The flow centrality algorithm follows the assignment specification exactly:
